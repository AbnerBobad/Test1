...
.PHONY: run
run: vet
	go run ./cmd/web -addr=${ADDRESS} -dsn=${WEBBASICS_DB_DSN}
...
db/migrations/up:
	...
	migrate -path ./migrations -database ${WEBBASICS_DB_DSN} up

# db/migrations/down-1: undo the last migration
.PHONY: db/migrations/down-1
db/migrations/down-1:
	@echo 'Running up migrations...'
	migrate -path ./migrations -database ${WEBBASICS_DB_DSN} down 1

## db/migrations/fix:  fix a SQL migration
.PHONY: db/migrations/fix
db/migrations/fix:
	@echo 'Checking migration status...'
	@migrate -path ./migrations -database $${WEBBASICS_DB_DSN} version > /tmp/migrate_version 2>&1
	...
	@if grep -q "dirty" /tmp/migrate_version; then \
		...
		migrate -path ./migrations -database $${WEBBASICS_DB_DSN} force $$version; \
